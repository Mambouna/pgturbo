...
class PGTurboGame:
    ...
    def load_handlers(self):
        ...
        spellcheck(vars(self.mod), list(self.EVENT_HANDLERS.values()))
        self.handlers = {}
    ...
    def run(self):
        ...
        try:
            self.mainloop()
        # We catch SystemExit here to prevent its traceback being printed if
        # the user put arguments in their on_exit() definition, which raises
        # the below TypeError and could thus confuse them.
        except SystemExit:
            pass
        finally:
    ...
    def inject_global_handlers(self):
        ...
        def key_down(event):
            ...
            if event.key == pygame.K_F12:
                try:
                    path = pgturbo.screen.screen_instance.screenshot()
